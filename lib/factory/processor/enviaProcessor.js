...
class EnviaProcessor {
    constructor(configuracoes) {
        ...
        this.soapAutorizacao = null;
        this.schemaXsdLote = null;
    ...
    async NFCeAssinaTransmite(xml) {
        ...
        try {
            this.configuraUrlsSefaz();
            let xmlAssinado = signature_1.Signature.signXmlX509(xml, "infNFe", this.configuracoes.certificado);
            let nfceObj = xmlHelper_1.XmlHelper.deserializeXml(xml, {
                explicitArray: false,
            });
            let nfce = nfceObj.NFe;
    ...
    async geraLoteXML(documento) {
        ...
        try {
            ...
            if (!valid) {
                throw new Error("Erro de validação do XML:\r\n" +
    ...
    configuraUrlsSefaz() {
        ...
        this.soapAutorizacao = Sefaz.getSoapInfo(empresa.endereco.uf, ambiente, nfe_1.ServicosSefaz.autorizacao);
    }
    // V2 não precisa deserializar o xml, economiza 0.004s~0.012s \o/
    // caso rodem bem em produção pode remover o metodo antigo
    appendQRCodeXML_V2(nfe, xmlAssinado) {
        let qrCode = null;
        ...
        if (isOffline) {
            ...
            let digestValue = nfe.signature.signedInfo.reference.digestValue.toString();
            qrCode = this.gerarQRCodeNFCeOffline(this.soapAutorizacao.urlQRCode, chave, "2", ambiente, diaEmissao, nfe.infNFe.total.ICMSTot.vNF, digestValue, this.configuracoes.empresa.idCSC, this.configuracoes.empresa.CSC);
        }
        else {
            qrCode = this.gerarQRCodeNFCeOnline(this.soapAutorizacao.urlQRCode, chave, "2", ambiente, this.configuracoes.empresa.idCSC, this.configuracoes.empresa.CSC);
        }
        let qrCodeObj = {
            qrCode: "<" + qrCode + ">",
    ...
    validaSchemaLote(xmlLote, nfe, result) {
        if (this.schemaXsdLote === null) {
            const pathXsd = path.join(__dirname, "..", "xsd/enviNFe_v4.00.xsd");
            const nfeProcXsd = fs.readFileSync(pathXsd, "utf8");
            process.chdir(path.dirname(pathXsd));
    ...
    async enviarNF(xml) {
        ...
        return await webserviceHelper_1.WebServiceHelper.makeSoapRequest(xml, certificado, this.soapAutorizacao, webProxy);
    }
    gerarXmlLote(xml) {
    ...
    gerarXml(documento) {
        ...
        let NFe = {
            ...
            infNFe: documento.docFiscal.modelo == "65"
    ...
    getDet(produtos, ambiente, modelo) {
        ...
        for (let i = 0; i < produtos.length; i++) {
            det_list.push({
                ...
                impostoDevol: produtos[i].prod.percentualDevolucao &&
    ...
    getDetProd(produto, ambiente, isPrimeiroProduto) {
        return {
            ...
            cEAN: produto.cEAN,
            xProd: ambiente == "2" && isPrimeiroProduto
    ...
    getImpostoIcms(icms) {
        return icms.CSOSN
            ? this.getImpostoIcmsSimplesNacional(icms)
            : this.getImpostoIcmsNormal(icms);
    }
    getImpostoIcmsNormal(icms) {
        switch (icms.CST) {
            case "00":
                return {
                    ICMS00: {
    ...
    getImpostoIcmsSimplesNacional(icms) {
        switch (icms.CSOSN) {
            case "101":
                return {
                    ICMSSN101: {
    ...
    getImpostoCOFINS(cofins, modelo) {
        ...
        switch (cofins.CST) {
            ...
            case "98":
            case "99":
                if (cofins.vBC + cofins.pCOFINS > 0 &&
    ...
    getIntermediador(intermediador) {
        if (!intermediador)
            return null;
        return {

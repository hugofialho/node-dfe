...
export class EnviaProcessor {
  ...
  public async executar(documento: NFeBase) {
    ...
    try {
      ...
      if (documento.docFiscal.modelo == "65") {
        let appendQRCode = this.appendQRCodeXML(
          documento as NFCeDocumento,
          xmlAssinado
        );
  ...
  private configuraUrlsSefaz() {
    ...
    this.soapAutorizacao = Sefaz.getSoapInfo(
      empresa.endereco.uf,
      ambiente,
  ...
  private gerarXml(documento: NFeBase) {
    ...
    let NFe = <schema.TNFe>{
      ...
      infNFe:
        documento.docFiscal.modelo == "65"
          ? this.gerarNFCe(documento as NFCeDocumento, dadosChave)
          : this.gerarNFe(documento as NFeDocumento, dadosChave),
    };
  ...
  private getIde(documento: DocumentoFiscal, dadosChave: any) {
    ...
    if (documento.indIntermed) {
      ide.indIntermed = Utils.getEnumByValue(
  ...
  private getImpostoIcmsNormal(icms: Icms) {
    switch (icms.CST) {
      case "00":
        return {
  ...
  private getImpostoIcmsSimplesNacional(icms: Icms) {
    switch (icms.CSOSN) {
      case "101":
        return {
  ...
  private getIntermediador(intermediador: Intermediador) {
    if (!intermediador) return null as any;
